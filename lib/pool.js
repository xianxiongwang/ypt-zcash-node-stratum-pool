...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        console.info("SubmitBlock:"+blockHex);

        _this.daemon.cmd(rpcCommand,
            rpcArgs,
            function (results) {
                console.info("SubmitBlock return:total"+results.length);

                for (var i = 0; i < results.length; i++) {
                    var result = results[i];

                    console.info("SubmitBlock return:"+JSON.stringify(result));
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    ...
                    console.log("new blockHex:["+blockHex+"]");
